...
function request(url: string, type?: string, withCredentials?: boolean, headers?: object): Promise<Blob | string | JSON | Document | XMLDocument> {
	var supportsURL = (typeof window != "undefined") ? window.URL : false; // TODO: fallback for url if window isn't defined
	var BLOB_RESPONSE: XMLHttpRequestResponseType = supportsURL ? "blob" : "arraybuffer";

	var deferred = defer<Blob | string | JSON | Document | XMLDocument>();
	...
	function handler(this: XMLHttpRequest) {
		if (this.readyState === XMLHttpRequest.DONE) {
			var responseXML: Document | null = null;
