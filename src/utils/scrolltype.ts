...
export default function scrollType(): string {
	...
	if (definer.scrollLeft > 0) {
		...
		if (definer.children[0] && definer.children[0].children[1] && 'scrollIntoView' in definer.children[0].children[1]) {
			(definer.children[0].children[1] as Element).scrollIntoView();
			if (definer.scrollLeft < 0) {
				type = "negative";
			}
		} else {
