...
class ContinuousViewManager extends DefaultViewManager {
	...
	display(section: Section, target?: string | number): Promise<void> {
		return DefaultViewManager.prototype.display.call(this, section, target)
			.then(function (this: ContinuousViewManager) {
				return this.fill();
			}.bind(this));
	}

	fill(_full?: any): Promise<void> {
		var full = _full || defer();
	...
	check(_offsetLeft?: number, _offsetTop?: number): Promise<boolean> {
		var checking = defer<boolean>();
		var newViews: Promise<View>[] = [];
		...
		let contentLength = horizontal ? (this.container?.scrollWidth || 0) : (this.container?.scrollHeight || 0);
	...
	trim(): Promise<void> {
		...
		var firstIndex = this.views?.indexOf(first) || 0;
		var lastIndex = this.views?.indexOf(last) || 0;
	...
	erase(view: View, above?: View[]): void { //Trim
	...
	addScrollListeners(): void {
		var scroller: Window | HTMLElement | undefined;
