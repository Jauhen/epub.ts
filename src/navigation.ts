...
class Navigation {
	 ...
	 * @param {document} xml navigation html / xhtml / ncx
	 */
	parse(xml: XMLDocument | any): void {
		let isXml = xml && typeof xml.nodeType === "number";
	 ...
	 * @return {object} navItem
	 */
	private getByIndex(target: string, index: number | undefined, navItems: NavItem[]): NavItem {
		if (!navItems || navItems.length === 0 || typeof index !== "number") {
			// @ts-expect-error: NavItem return type
			return undefined;
		}
	 ...
	 * @return {object} navItem
	 */
	private navItem(item: Element, parent?: string): NavItem {
		let id = item.getAttribute("id") || undefined;
		let content = (filterChildren(item, "a", true) as Element | null)
		...
		let nested = filterChildren(item, "ol", true) as Element | null;
		if (nested) {
	...
	private landmarkItem(item: Element): LandmarkItem {
		...
		if (!content) {
			// @ts-expect-error: LandmarkItem return type
			return undefined;
		}
	...
	private ncxItem(item: Element): NavItem {
		...
		if (
			parentNode &&
			(parentNode.nodeName === "navPoint" || parentNode.nodeName.split(":").slice(-1)[0] === "navPoint")
		) {
