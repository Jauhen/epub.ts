...
import { extend, defer, Defer } from "./utils/core";
import Url from "./utils/url";
import Path from "./utils/path";
import Spine from "./spine";
import Locations from "./locations";
import Container from "./container";
import Packaging, { PackagingManifestObject, PackagingMetadataObject, PackagingObject } from "./packaging";
...
class Book {
	...
	constructor(url?: string | ArrayBuffer | Blob | BookOptions, options?: BookOptions) {
		...
		if (url) {
			this.open(url as string | ArrayBuffer, this.settings.openAs).catch((error: any) => {
	...
	open(input: string | ArrayBuffer, what?: string): Promise<any> {
		...
		if (type === INPUT_TYPE.BINARY) {
			...
			this.archived = true;
			this.url = new Url("/", "");
			opening = this.openEpub(input as ArrayBuffer, true);
	 ...
	 * @return {Promise}
	 */
	openEpub(data: ArrayBuffer | string, encoding?: boolean): Promise<any> {
	...
	openContainer(url: string): Promise<string> {
		return this.load(url)
			.then((xml: any) => {
				...
				return this.resolve((this.container as Container).packagePath!);
			});
	}
	...
	openManifest(url: string): Promise<any> {
		...
		return this.load(url)
			.then((json: any) => {
				...
				(this.packaging as any).load(json);
				return this.unpack(this.packaging);
			});
	}
	...
	unpack(packaging: Packaging): void {
		(this as any).package = packaging; // deprecated
		if (this.packaging && this.packaging.metadata.layout === "") {
			// rendition:layout not set - check display options if book is pre-paginated
			this.load(this.url!.resolve(IBOOKS_DISPLAY_OPTIONS_PATH)).then((xml: any) => {
	...
	loadNavigation(packaging: Packaging): Promise<Navigation> {
		const navPath = (packaging as any).navPath || (packaging as any).ncxPath;
		const toc = (packaging as any).toc;
		if (toc) {
	...
	store(name: string): Store {
		...
		this.opened.then(() => {
			...
			if (this.resources) {
				this.resources.settings.replacements = replacementsSetting?.toString() || "blobUrl";
	...
	key(identifier?: string): string {
		const ident = identifier || (this.packaging && this.packaging.metadata.identifier) || (this.url && this.url.filename);
