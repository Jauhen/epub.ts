...
class Contents {
	...
	mediaQueryListeners() {
		...
		var mediaChangeHandler = (m: MediaQueryList) => {
		...
		for (var i = 0; i < sheets.length; i += 1) {
			...
			for (var j = 0; j < rules.length; j += 1) {
				   ...
				   if ("media" in rules[j] && (rules[j] as CSSMediaRule).media) {
					   ...
					   var mql = this.window!.matchMedia(mediaRule.media.mediaText);
					   mql.addListener(function(this: MediaQueryList, ev: MediaQueryListEvent) {
	...
	locationOf(target: string | EpubCFI, ignoreClass?: string): { left: number; top: number } {
		var position: DOMRect | undefined;
			...
			if(range) {
				try {
					if (!range.endContainer ||
						...
						let pos = (range.startContainer as Text).textContent?.indexOf(" ", range.startOffset) ?? -1;
						if (pos == -1) {
	...
	addStylesheetRules(rules: any[] | Record<string, any>, key?: string): void {
		...
		if (Object.prototype.toString.call(rules) === "[object Array]") {
			...
			for (var i = 0, rl = rulesArray.length; i < rl; i++) {
				var j = 1, rule = rulesArray[i], selector = rulesArray[i][0], propStr = "";
	...
	addEventListeners(): void {
		...
		DOM_EVENTS.forEach((eventName) => {
			if (this._triggerEvent) {
				this.document!.addEventListener(eventName as keyof DocumentEventMap, this._triggerEvent as EventListener, { passive: true } as any);
			}
		});
	...
	removeEventListeners(): void {
		...
		DOM_EVENTS.forEach((eventName) => {
			if (this._triggerEvent) {
				this.document!.removeEventListener(eventName as keyof DocumentEventMap, this._triggerEvent as EventListener, { passive: true } as any);
			}
		});
	...
	removeSelectionListeners(): void {
		...
		if (this._onSelectionChange) {
			this.document.removeEventListener("selectionchange", this._onSelectionChange as EventListener);
		}
	...
	triggerSelectedEvent(selection: Selection | null): void {
		var range: Range, cfirange: EpubCFI;
	...
	columns(width: number, height: number, columnWidth: number, gap: number, dir?: string): void {
		...
		this.viewport({ width: width.toString(), height: height.toString(), scale: "1.0", scalable: "no", minimum: "1.0", maximum: "1.0" });
	...
	fit(width: number, height: number, section?: any): void {
		...
		var viewportWidth = parseInt(viewport.width || "0");
		var viewportHeight = parseInt(viewport.height || "0");
	...
	writingMode(mode?: string): string {
		...
		return this.window?.getComputedStyle(this.documentElement!)![WRITING_MODE as any] || '';
	}
