...
class Packaging {
	 ...
	 * @return {object} spine
	 */
	private parseSpine(spineXml: Node, manifest: PackagingManifestObject): Array<SpineItem> {
	 ...
	 * @return {string}
	 */
	private findNcxPath(manifestNode: Element, spineNode: Element): string | false {
	...
	private getElementText(xml: Node, tag: string): string {
		...
		const found = (xml as Element).getElementsByTagNameNS("http://purl.org/dc/elements/1.1/", tag);
