...
class Rendition {
	...
	_layout: any;
	starting: any;
	// EventEmitter methods (added at runtime)
	...
	start(): void {
		...
		if (!this.settings.layout && (bookAny.package?.metadata?.layout === "pre-paginated" || bookAny.displayOptions?.fixedLayout === "true")) {
		...
		this.direction(bookAny.package?.metadata?.direction || this.settings.defaultDirection);

		// Parse metadata to get layout props
		this.settings.globalLayoutProperties = this.determineLayoutProperties(bookAny.package?.metadata || {});
	...
	_display(target?: string | number): Promise<any> {
		...
		const isCfiString = this.epubcfi.isCfiString(typeof target === 'string' ? target : '');
		...
		if ((this.book as any).locations.length() && typeof target === 'string' && isFloat(target)) {
			target = (this.book as any).locations.cfiFromPercentage(parseFloat(target));
		}
	...
	*/
	...
	determineLayoutProperties(metadata: any): any {
		...
		var orientation = (this.settings as any).orientation || metadata.orientation || "auto";
		var flow = this.settings.flow || metadata.flow || "auto";
		var viewport = metadata.viewport || "";
		var minSpreadWidth = this.settings.minSpreadWidth || metadata.minSpreadWidth || 800;
		var direction = (this.settings as any).direction || metadata.direction || "ltr";

		if (((typeof this.settings.width === 'number' && (this.settings.width === 0 || this.settings.width > 0)) ||
			(typeof this.settings.width === 'string' && this.settings.width.length > 0)) &&
			((typeof this.settings.height === 'number' && (this.settings.height === 0 || this.settings.height > 0)) ||
				(typeof this.settings.height === 'string' && this.settings.height.length > 0))) {
	 ...
	 */
	reportLocation() {
		return this.q.enqueue(function reportedLocation(this: Rendition) {
	...
	injectStylesheet(doc: Document, section: any): void {
		...
		style.setAttribute("href", this.settings.stylesheet ? String(this.settings.stylesheet) : "");
	...
	injectScript(doc: Document, section: any): void {
		...
		script.setAttribute("src", this.settings.script ? String(this.settings.script) : "");
