...
export class Section {
	 ...
	 */
	load(_request?: (url: string) => Promise<Document>): Promise<Document | undefined> {
	 ...
	 */
	render(_request?: (url: string) => Promise<Document>): Promise<string | undefined> {
		const rendering = defer<string | undefined>();
		const rendered = rendering.promise;
		this.load(_request)
			.then(() => {
				const userAgent = (typeof navigator !== 'undefined' && (navigator as any).userAgent) || '';
		...
		const find = function (node: Element | Text) {
			...
			while ((pos = text.indexOf(query, last + 1)) !== -1) {
				...
				if (node.textContent.length < limit) {
					...
					excerpt = node.textContent.substring(Math.max(0, pos - limit / 2), pos + limit / 2);
					excerpt = "..." + excerpt + "...";
				}
	 ...
	 * @return {object[]} A list of matches, with form {cfi, excerpt}
	 */
	search(_query: string, maxSeqEle: number = 5): Array<{ cfi: string; excerpt: string }> {
		...
		const search = function (nodeList: Array<Text>) {
			...
			if (pos !== -1) {
				...
				if (pos < (nodeList[startNodeIndex].textContent?.length || 0)) {
					...
					const beforeEndLengthCount = nodeList.slice(0, endNodeIndex).reduce((acc, current) => acc + (current.textContent?.length || 0), 0);
					range.setEnd(endNode, beforeEndLengthCount > endPos ? endPos : endPos - beforeEndLengthCount);
					const cfi = section.cfiFromRange(range);
					let excerpt = nodeList.slice(0, endNodeIndex + 1).reduce((acc, current) => acc + (current.textContent || ""), "");
					if (excerpt.length > excerptLimit) {
						excerpt = excerpt.substring(Math.max(0, pos - excerptLimit / 2), pos + excerptLimit / 2);
						excerpt = "..." + excerpt + "...";
					}
					matches.push({ cfi, excerpt });
				}
		...
		const treeWalker = (this.document as any).createTreeWalker(this.document, NodeFilter.SHOW_TEXT);
		let node: Text | null;
