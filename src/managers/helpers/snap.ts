...
class Snap {
	...
	touchCanceler: boolean = false;
	resizeCanceler: boolean = false;
	...
	// EventEmitter methods (initialized by EventEmitter mixin)
	...
	onTouchMove(e: TouchEvent) {
		let { screenX, screenY } = e.touches[0];
		let deltaY = this.endTouchY !== undefined ? Math.abs(screenY - this.endTouchY) : 0;
	...
	wasSwiped(): number {
		...
		if (this.endTouchX === undefined || this.startTouchX === undefined || 
		    this.endTime === undefined || this.startTime === undefined) {
			return 0;
		}
