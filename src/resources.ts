...
class Resources {
	...
	private split(): void {
		// HTML
		this.html = this.resources.filter((item: ResourceItem) =>
			item.type === "application/xhtml+xml" || item.type === "text/html"
		);
		// Exclude HTML
		this.assets = this.resources.filter((item: ResourceItem) =>
			item.type !== "application/xhtml+xml" && item.type !== "text/html"
		);
		// Only CSS
		this.css = this.resources.filter((item: ResourceItem) =>
			item.type === "text/css"
		);
	}
	...
	createUrl(url: string): Promise<string> {
		...
		if (this.settings.archive) {
			...
			if (this.settings.replacements === "base64") {
				...
				return this.settings.request(url, 'blob').then((blob: Blob) => createBlobUrl(blob, mimeType) as string);
			}
		}
	...
	replacements(): Promise<string[]> {
		...
		const replacements = this.urls.map((url: string) => {
			const absolute = this.settings.resolver ? this.settings.resolver(url) : url;
		...
		return Promise.all(replacements)
			.then((replacementUrls: (string | null)[]) => {
				this.replacementUrls = replacementUrls.filter((url): url is string => typeof url === "string");
	 ...
	 * @return {Promise}
	 */
	replaceCss(archive?: Archive, resolver?: (url: string) => string): Promise<string[]> {
	...
	private createCssFile(href: string): Promise<string> {
		...
		const absolute = this.settings.resolver ? this.settings.resolver(href) : href;
		// Get the text of the css file from the archive
		let textResponse: Promise<string>;
		if (this.settings.archive) {
			textResponse = this.settings.archive.getText(absolute) || Promise.resolve("");
		} else if (this.settings.request) {
		...
		const relUrls = this.urls.map((assetHref: string) => {
			const resolved = this.settings.resolver ? this.settings.resolver(assetHref) : assetHref;
