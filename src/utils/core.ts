 ...
 * @memberof Core
 */
export const requestAnimationFrame = (typeof window != "undefined") ? (window.requestAnimationFrame || (window as any).mozRequestAnimationFrame || (window as any).webkitRequestAnimationFrame || (window as any).msRequestAnimationFrame) : false;
const ELEMENT_NODE = 1;
const TEXT_NODE = 3;
const COMMENT_NODE = 8;
const DOCUMENT_NODE = 9;
const _URL = typeof URL != "undefined" ? URL : (typeof window != "undefined" ? (window.URL || (window as any).webkitURL || (window as any).mozURL) : undefined);
...
export function insert(item: any, array: Array<any>, compareFunction: Function): number {
	var location = locationOf(item, array, compareFunction, 0 as any, array.length as any);
 ...
 * @memberof Core
 */
export function locationOf(item: any, array: Array<any>, compareFunction: Function, _start?: number, _end?: number): number {
	var start = _start || 0;
 ...
 * @memberof Core
 */
export function indexOfSorted(item: any, array: Array<any>, compareFunction: Function, _start?: number, _end?: number): number {
	var start = _start || 0;
	var end = _end || array.length;
	var pivot = parseInt(String(start + (end - start) / 2));
	var compared;
	...
	if(compared === -1) {
		return indexOfSorted(item, array, compareFunction, pivot as any, end as any);
	} else{
		return indexOfSorted(item, array, compareFunction, start as any, pivot as any);
	}
}
/**
 ...
 * @memberof Core
 */
export function qsp(el: Element | Document, sel: string, props: any): Element | null {
	var q, filtered;
	if (typeof (el as any).querySelector != "undefined") {
		sel += "[";
...
export function treeWalker(root: Node, func: Function, filter: number): void {
	var treeWalker = document.createTreeWalker(root, filter);
	let node;
 ...
 * @memberof Core
 */
export function querySelectorByType(html: Element | Document, element: string, type: string): Element | null {
	var query;
	...
	if(!query || (query as any).length === 0) {
		let queryAll = qsa(html, element);
		for (var i = 0; i < queryAll.length; i++) {
			if((queryAll[i] as Element).getAttributeNS("http://www.idpf.org/2007/ops", "type") === type ||
 ...
 * @memberof Core
 */
export function filterChildren(el: Element, nodeName: string, single: boolean): Element[] | Element | void {
	var result: Element[] = [];
	var childNodes = el.childNodes;
	for (var i = 0; i < childNodes.length; i++) {
		let node = childNodes[i];
 ...
 * @memberof Core
 */
export function getParentByTagName(node: Node, tagname: string): Element | void {
	let parent;
...
export class RangeObject implements Partial<Range> {
	...
	selectNodeContents(referenceNode: Node): void {
		...
		let endIndex = (referenceNode.nodeType === 3) ?
				(referenceNode as Text).textContent!.length : referenceNode.childNodes.length;
