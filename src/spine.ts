...
class Spine {
	 ...
	 */
	unpack(_package: Packaging, resolver: (href: string, abs?: boolean) => string, canonical: (href: string) => string): void {
		// @ts-ignore
		...
		// @ts-ignore
		this.baseUrl = (_package as any).baseUrl || (_package as any).basePath || "";
		this.length = this.items.length;

		this.items.forEach((item: any, index: number) => {
			const manifestItem = this.manifest ? this.manifest[item.idref] : undefined;
			let spineItem;

			item.index = index;
			item.cfiBase = this.epubcfi.generateChapterComponent(this.spineNodeIndex ?? 0, item.index, item.id);
