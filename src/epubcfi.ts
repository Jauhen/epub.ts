...
class EpubCFI {
	...
	constructor(cfiFrom?: string | Range | Node | EpubCFI, base?: string | EpubCFIComponent, ignoreClass?: string) {
		...
		if (type === "string") {
			...
			return Object.assign(this, this.fromRange(cfiFrom as Range, this.base, ignoreClass || ""));
		} else if (type === "node") {
			return Object.assign(this, this.fromNode(cfiFrom as Node, this.base, ignoreClass));
	...
	checkType(cfi?: string | Range | Node | EpubCFI): string | false {
		if (this.isCfiString(cfi)) {
			return "string";
			// Is a range object
		} else if (cfi && typeof cfi === "object" && (type(cfi) === "Range" || typeof ((cfi as any).startContainer) != "undefined")) {
			return "range";
		} else if (cfi && typeof cfi === "object" && typeof ((cfi as any).nodeType) != "undefined") { // || typeof cfi === "function"
			return "node";
	...
	parseComponent(componentStr?: string): EpubCFIComponent {
		...
		component.steps = steps.map((step: string) => this.parseStep(step)).filter((step) => step !== undefined) as EpubCFIStep[];

		return component;
	}

	parseStep(stepStr: string): EpubCFIStep | undefined {
		var type: string, num: number, index: number, has_brackets: RegExpMatchArray | null, id: string | undefined;
	...
	parseTerminal(termialStr: string): { offset: number, assertion: string } {
		let characterOffset: number | null = null;
		let textLocationAssertion: string | null = null;
		var assertion: RegExpMatchArray | null = termialStr.match(/\[(.*)\]/);
	...
	getPathComponent(cfiStr: string): string | undefined {
		...
		var indirection: string[] = cfiStr.split("!");

		if (indirection[1]) {
			let ranges: string[] = indirection[1].split(",");
			return ranges[0];
		}
	...
	getRange(cfiStr: string): string[] | false {
		...
		var ranges: string[] = cfiStr.split(",");
	...
	getCharecterOffsetComponent(cfiStr: string): string {
		var splitStr: string[] = cfiStr.split(":");
		return splitStr[1] || "";
	}
	...
	joinSteps(steps: Array<EpubCFIStep>): string {
		...
		return steps.map(function (part: EpubCFIStep): string {
			var segment: string = "";
	...
	compare(cfiOne: string | EpubCFI, cfiTwo: string | EpubCFI): number {
		...
		if (cfiOne.range) {
			...
			terminalA = cfiOne.start?.terminal || { offset: 0, assertion: "" };
		} else {
		...
		if (cfiTwo.range) {
			...
			terminalB = cfiTwo.start?.terminal || { offset: 0, assertion: "" };
		} else {
	...
	step(node: Element): EpubCFIStep {
		var nodeType: string = (node.nodeType === TEXT_NODE) ? "text" : "element";
	...
	filteredStep(node: Element, ignoreClass: string): EpubCFIStep | undefined {
		var filteredNode: Element | false = this.filter(node, ignoreClass);
		var nodeType: string;
	...
	pathTo(node: Element, offset?: number | null, ignoreClass?: string): EpubCFISegment {
		var segment: EpubCFISegment = {
			steps: [],
			terminal: {
				offset: 0,
				assertion: ""
		...
		var currentNode: Element | null = node;
	...
	fromRange(range: Range, base: string | object, ignoreClass: string): any {
		...
		var startOffset: number = range.startOffset;
		var endOffset: number = range.endOffset;

		var needsIgnoring: boolean = false;
	...
	filter(anchor: Node, ignoreClass: string): Element | false {
		var needsIgnoring: boolean;
		var sibling: Node | null = null;  // to join with
		var parent: Node | null = null, previousSibling: Node | null, nextSibling: Node | null;
	...
	patchOffset(anchor: Node, offset: number, ignoreClass: string): number {
		...
		var totalOffset: number = offset;

		// If the parent is a ignored node, get offset from it's start
		if (anchor.parentNode && (anchor.parentNode as Element).classList.contains(ignoreClass)) {
	...
	normalizedMap(children: NodeList | HTMLCollection, nodeType: number, ignoreClass: string): { [key: number]: number } {
		var output: { [key: number]: number } = {};
		var prevIndex: number = -1;
		var i: number, len: number = children.length;
		var currNodeType: number;
		var prevNodeType: number = -1;
	...
	position(anchor: Node): number {
		var children: Node[], index: number;
	...
	filteredPosition(anchor: Node, ignoreClass: string): number {
		var children: NodeList | HTMLCollection, index: number, map: { [key: number]: number };
	...
	stepsToXpath(steps: Array<EpubCFIStep>): string {
		...
		steps.forEach(function (step: EpubCFIStep): void {
			var position: number = step.index + 1;
	...
	*/
	...
	walkToNode(steps: Array<EpubCFIStep>, _doc?: Document, ignoreClass: string = ''): Node | null {
		var doc: Document = _doc || document;
		var container: Element | null = doc.documentElement;
		var children: NodeList | HTMLCollection;
		var step: EpubCFIStep;
		var len: number = steps.length;
		var i: number;
		...
		for (i = 0; i < len; i++) {
			...
			if (step.type === "element") {
				...
				container = this.textNodes(container, ignoreClass)[step.index] as Element;
			}
			if (!container) {
	...
	findNode(steps: Array<EpubCFIStep>, _doc?: Document, ignoreClass?: string): Node | null {
		var doc: Document = _doc || document;
	...
	fixMiss(steps: Array<EpubCFIStep>, offset: number, _doc?: Document, ignoreClass: string = ''): any {
		var container: Node | null = this.findNode(steps.slice(0, -1), _doc, ignoreClass);
		var children: NodeList = container!.childNodes;
		var map: { [key: number]: number } = this.normalizedMap(children, TEXT_NODE, ignoreClass);
		var child: Node;
		var len: number;
		var lastStepIndex: number = steps[steps.length - 1].index;
	...
	toRange(_doc?: Document, ignoreClass?: string): Range | null {
		var doc: Document = _doc || document;
		var range: Partial<Range>  = new RangeObject();
		var start: EpubCFIComponent | null, end: EpubCFIComponent | null, startContainer: Node | null, endContainer: Node | null = null;
		var cfi: EpubCFI = this;
		var startSteps: Array<EpubCFIStep>, endSteps: Array<EpubCFIStep>;
		var needsIgnoring: boolean = ignoreClass ? (doc.querySelector("." + ignoreClass) != null) : false;
		var missed: any;
		...
		if (cfi.range) {
			...
			startContainer = this.findNode(startSteps, doc, needsIgnoring ? ignoreClass : '');
			end = cfi.end;
			endSteps = cfi.path.steps.concat(end!.steps);
			endContainer = this.findNode(endSteps, doc, needsIgnoring ? ignoreClass : '');
		} else {
			start = cfi.path;
			startSteps = cfi.path.steps;
			startContainer = this.findNode(cfi.path.steps, doc, needsIgnoring ? ignoreClass : '');
		}
		...
		if (startContainer) {
			try {
				...
				missed = this.fixMiss(startSteps, start!.terminal.offset, doc, needsIgnoring ? ignoreClass : '');
		...
		if (endContainer) {
			try {
				...
				missed = this.fixMiss(endSteps!, cfi.end!.terminal.offset, doc, needsIgnoring ? ignoreClass : '');
