...
class IframeView implements View {
	...
	expand(force?: boolean): void {
		...
		// Expand Horizontally
			...
			if (this.settings.flow === "paginated" && height && this.layout.height &&
	...
	reframe(width?: number, height?: number): void {
		var size: { width?: number, height?: number, widthDelta?: number, heightDelta?: number };
	...
	onLoad(event: Event, promise: { resolve: (value: Contents | undefined) => void, reject: (reason?: any) => void }): void {
		if (!this.iframe) {
		...
		this.document = this.iframe.contentDocument || undefined;
		if (this.document && this.document.body && this.section && this.section.cfiBase !== undefined && this.section.index !== undefined) {
	...
	onResize(view: View, size: { width?: number, height?: number, widthDelta?: number, heightDelta?: number }): void {
		// Stub, override with a custom functions
	}
	...
	highlight(
		...
		let h = this.pane.addMark(m) as Highlight;
	...
	underline(
		...
		let h = this.pane.addMark(m) as Underline;
	...
	placeMark(element: HTMLElement, range: Range): void {
		let top: number | undefined, right: number | undefined, left: number | undefined;
		if (this.layout.name === "pre-paginated" ||
