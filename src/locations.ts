...
class Locations {
	...
	private createRange(): { startContainer: Element | undefined, startOffset: number | undefined, endContainer: Element | undefined, endOffset: number | undefined } {
		return {
	...
	locationFromCfi(cfi: string | EpubCFI): number {
		...
		loc = locationOf(cfi, this._locations, (this.epubcfi as any).compare, 0, this._locations.length - 1);
	...
	cfiFromPercentage(percentage: number): string {
		...
		if (percentage >= 1) {
			let cfi = new EpubCFI(this._locations ? this._locations[this.total as number] : '');
			cfi.collapse();
			return cfi.toString();
		}
