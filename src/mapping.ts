...
class Mapping {
	...
	private walk(root: Node, func: Function): any {
		...
		var filter = {
			acceptNode: function(node: Node): number {
				if (node.nodeType === Node.TEXT_NODE && (node as Text).data && (node as Text).data.trim().length > 0) {
		...
		var safeFilter: NodeFilter = filter.acceptNode as any;
	...
	private findTextStartRange(node: Node, start: number, end: number): Range {
		...
		var range: Range;
		var pos: DOMRect;
		var left: number, top: number, right: number;
