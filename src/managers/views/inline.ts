...
class InlineView {
	...
	render(request?: (url: string) => Promise<Document>, show?: boolean): Promise<void> {
		...
		return contents.then((resolved) => {this.load(resolved);})
			// .then(function(doc){
			...
			.then(() => {
		       ...
		       if (this.settings.layout && this.contents) {
			       this.settings.layout.format(this.contents, this.section, this.settings.axis);
		       }
	...
	size(_width?: number, _height?: number): void {
		var width = _width || this.settings.width || 0;
		var height = _height || this.settings.height || 0;
	...
	expand(force?: boolean): void {
		...
		var textWidth: number | undefined, textHeight: number | undefined;
	...
	resize(width?: number, height?: number): void {
       ...
       onDisplayed(view: this): void {
	       // Stub, override with a custom functions
       }

       onResize(view: this, e: any): void {
	       // Stub, override with a custom functions
       }
